...
Usage:
    scan.py plugins
    scan.py info <插件名称>
    scan.py scan -n <线程数量> -t <URL> -p <插件名称>...
    scan.py scan -n <线程数量> -f <文件路径> -p <插件名称>...
...
class WebScan(object):
    ...
    def __init__(self):
        ...
        """初始化方法"""
        self.plugins = {}  # 用于存储加载的插件
        self.plugin_confs = self.get_plugins_conf()  # 获取插件的配置信息
    ...
    def exec_plugin_threads(self, plugin, targets, thread_num):
        ...
        # 为每个目标创建一个线程
        for u_id, url in enumerate(targets):
            threads.append(MyThread(self.exec_plugin_single_thread, (plugin, url, u_id)))
...
def main():
    ...
    if args["plugins"]:
        ...
        plugin = args["<插件名称>"][0]  # 获取插件名
        ts.info_plugin(plugin)  # 显示插件信息
    elif args["scan"]:
        thread_num = int(args["<线程数量>"])  # 获取线程数
        if args["-t"]:
            targets = [args["<URL>"]]  # 获取目标域名
        elif args["-f"]:
            targets = [target.strip() for target in open(args["<文件路径>"], encoding="utf-8") if not target.isspace()]  # 从文件中读取目标域名
        plugins = args["<插件名称>"]  # 获取插件列表
        print(plugins)
