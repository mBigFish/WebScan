    ...
    def get_ipinfo(self, domain):
        try:
            ...
            if not ipaddr:
                ...
                if msg:
                    msg = msg[:-1] + "(open)"
                res = {'Hostname': domain, 'IPAddr': ipaddr, 'ISP': ipinfo['isp'], 'Country': ipinfo['country'],
                       'asn_cidr': ipinfo['asn_cidr'], 'asn_description': ipinfo['asn_description'], 'PortScan': msg,
                       'StatusCode': status_code, 'httpserver': httpserver, 'Title': title}
...
def save2csv(data, filename):
    with open(filename, 'w', newline='', encoding='utf-8-sig') as csvfile:
        fieldnames = ['Hostname', 'IPAddr', 'Country', 'ISP', 'asn_cidr', 'asn_description', 'PortScan', 'StatusCode',
                      'httpserver', 'Title']
...
def iplookup(ipaddr):
    try:
        ...
        return {'country': 'Unknown', 'isp': 'Unknown', 'asn_cidr': 'Private-Use Networks',
    ...
    if info['nets'][0]['description']:
        ...
        ipinfo = {'country': countryname, 'isp': temp[0], 'asn_cidr': info['asn_cidr'],
                  'asn_description': info['asn_description']}
    else:
        ipinfo = {'country': countryname, 'isp': 'Not Found', 'asn_cidr': info['asn_cidr'],
                  'asn_description': info['asn_description']}
    return ipinfo
...
def get_args():
    ...
    parser = argparse.ArgumentParser('domain2ip.py',
                                     formatter_class=lambda prog: argparse.HelpFormatter(prog, max_help_position=40))
    parser.add_argument('-w', '--wordlist', help='Lines contain domain names, one per line', dest='wordlist',
                        required=False, default="domains.txt")
    parser.add_argument('-p', '--port', help='Ports to scan', dest='ports', required=False, default="80,443")
    parser.add_argument('-t', '--threads', help='Number of threads', dest='threads', required=False, type=int,
                        default=8)
    parser.add_argument('-o', '--output', help="Write output to a csv file", dest='output_filename', required=False,
                        default="output.csv")
    parser.add_argument('-w3', '--www', action="store_true", default=False, help='Add www to domain', dest='w3',
                        required=False)
